...
+ (BOOL)saveImageWithImageData:(NSData *)imageData andName:(NSString *)imageName {
    if ([LXF_FileManager createDirInCache:@"LXF_ImageCache"]) {
        return [LXF_FileManager saveDataToCacheDir:[LXF_FileManager pathInCacheDirectory:@"LXF_ImageCache"]
                                          data:imageData
                                      dataName:[LXF_FileManager removeSpecialWithStr:imageName andMd5:NO]];
    }
    return NO;
}

+ (NSData *)fetchImageDataWithImageName:(NSString *)imageName {
    NSData *data = [LXF_FileManager loadDataWithPath:[LXF_FileManager pathInCacheDirectory:@"LXF_ImageCache"]
                                            name:[LXF_FileManager removeSpecialWithStr:imageName andMd5:NO]];
    if (data) {
        return data;
    }
    return nil;
