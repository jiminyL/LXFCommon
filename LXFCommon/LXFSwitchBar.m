...
- (instancetype)initBackgroundStyleWithNameArray:(NSArray<NSString *> *)nameArray andTextColor:(UIColor *)textColor andBgColor:(UIColor *)bgColor
{
    if (self = [super init]) {
        ...
        for (int i=0; i<nameArray.count; i++) {
            ...
            [button setBackgroundImage:[self.class createImageWithColor:bgColor frame:CGRectMake(0.f, 0.f, 10.f, 10.f)] forState:UIControlStateSelected];
            [button setBackgroundImage:[self.class createImageWithColor:[UIColor clearColor] frame:CGRectMake(0.f, 0.f, 10.f, 10.f)] forState:UIControlStateNormal];
...
- (instancetype)initNikeStyleWithNameArray:(NSArray<NSString *> *)nameArray andUnseletedImage:(UIImage *)unSeletedImage andSeletedImage:(UIImage *)seleteImage andTextColor:(UIColor *)textColor andImageAtRight:(BOOL)imageAtRight andScrollType:(BOOL)scrollType
{
    if (self = [super init]) {
        ...
        for (int i=0; i<nameArray.count; i++) {
            ...
            CGFloat titleWidth = [self widthForHeight:20.f font:[UIFont systemFontOfSize:16.f] string:titleStr];
            if (imageAtRight) {
...
- (void)setFrame:(CGRect)frame
{
    ...
    if (self.scrollType) {
        ...
        if (self.scrollView) {
            ...
            [self.scrollView setContentSize:CGSizeMake(offsetX, self.frame.size.height)];
...
- (void)setCurrentIndex:(NSInteger)currentIndex
{
    ...
    if (self.styleType == LXFSwitchBarStyleTypeBottomLine) {
        ...
        for (NSString *str in self.nameArray) {
            CGFloat temp = [self widthForHeight:self.frame.size.height font:self.font string:str];
            if (temp > width) {
                width = temp;
            }
        }
...
@interface LXFSwitchBarButton()


@end

@implementation LXFSwitchBarButton
